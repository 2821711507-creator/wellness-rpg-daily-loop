...
<span>{template.title} · {template.minutes}분 · {template.intensity === 'easy' ? '가볍게' : '보통 강도'}</span></label>)}</fieldset> : <label>이동할 날짜<select aria-label="이동할 날짜" value={target} onChange={event => setTarget(event.target.value)}>{getWeekDateKeys(plan.weekStart).map(date => <option value={date} key={date}>{date}</option>)}</select></label>}{message && <p role="alert" className="dialog-error">{message}</p>}<div className="dialog-actions"><button className="secondary-button" onClick={close}>취소</button><button className="primary-button" onClick={apply}>{action.kind === 'replace' ? '이 운동으로 교체' : '이동하기'}</button></div></section></div>}</div>
}
