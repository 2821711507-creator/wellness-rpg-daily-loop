...
export function TodayScreen({ state, setSmoothie, setActivity, complete, setBase, onOpenPlan, now = () => new Date() }: { state: WellnessState; setSmoothie:(v:SmoothieItem[])=>void; setActivity:(id:string)=>void; complete:(id:string)=>void; setBase:(b:AvatarBase)=>void; onOpenPlan:()=>void; now?:()=>Date }) {
  const today = now()
