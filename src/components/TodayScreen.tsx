...
import { formatKoreanDate, getMonday, toLocalDateKey, type PlanMutationResult } from '../domain/weeklyPlan'
...
export function TodayScreen({ state, setSmoothie, setActivity, replaceActivity, complete, onOpenPlan, onOpenRecords, onOpenAvatar, onOpenMore, customizeButtonRef, now = () => new Date(), account }: { state: WellnessState; setSmoothie:(v:SmoothieItem[])=>void; setActivity:(id:string)=>void; replaceActivity?:(id:string, templateId:string)=>PlanMutationResult; complete:(id:string)=>void; onOpenPlan:()=>void; onOpenRecords:()=>void; onOpenAvatar:()=>void; onOpenMore:()=>void; customizeButtonRef?:Ref<HTMLButtonElement>; now?:()=>Date; account?:TodayAccount }) {
  const today = now()
  ...
  const next = () => {
    const nextTemplateId = rotationCandidates[(rotationCandidates.indexOf(activity) + 1) % rotationCandidates.length].id
    // When a weekly plan covers today, `activity` comes from `plannedActivity`, not
    // `state.selectedActivityId` -- calling `setActivity` alone would update state
    // that has no effect on what's displayed, making the button appear inert.
    if (plannedActivity && replaceActivity) replaceActivity(plannedActivity.id, nextTemplateId)
    else setActivity(nextTemplateId)
  }
