...
export function TodayScreen({ state, setSmoothie, setActivity, complete, onOpenPlan, onOpenRecords, onOpenAvatar, customizeButtonRef, now = () => new Date() }: { state: WellnessState; setSmoothie:(v:SmoothieItem[])=>void; setActivity:(id:string)=>void; complete:(id:string)=>void; onOpenPlan:()=>void; onOpenRecords:()=>void; onOpenAvatar:()=>void; customizeButtonRef?:Ref<HTMLButtonElement>; now?:()=>Date }) {
  const today = now()
