...
export function App() {
  const game = useWellnessGame()
  const [view, setView] = useState<'today' | 'plan'>('today')
  if (!game.state.profile) return <Onboarding onComplete={game.onboard}/>
  if (view === 'today') return <><TodayScreen state={game.state} setSmoothie={game.setSmoothie} setActivity={game.setActivity} complete={game.complete} setBase={game.setBase} onOpenPlan={() => setView('plan')}/>{game.warning && <p role="status">{game.warning}</p>}</>
