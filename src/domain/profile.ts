...
export interface UserProfile { age: number; heightCm: number; weightKg: number; calculationSex: CalculationSex; activityLevel: ActivityLevel; goal: Goal; cutIntensity?: CutIntensity }
...
type LegacyOrCurrentProfile = Omit<UserProfile, 'goal' | 'cutIntensity'> & Partial<Pick<UserProfile, 'goal' | 'cutIntensity'>>

export function normalizeProfile(value: LegacyOrCurrentProfile | null): UserProfile | null {
  if (value === null) return null
  const goal: Goal = value.goal ?? 'cut'
  return { ...value, goal, ...(goal === 'cut' ? { cutIntensity: value.cutIntensity ?? 'mild' } : {}) }
}
